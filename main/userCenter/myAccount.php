...
$accountType = "充值";
$sql = "SELECT * FROM accounts  WHERE accountType ='$accountType'AND user_name='$username' order by  created_at desc";
...
//page = 3 limit 6
/*用户名*/
$sql = "select * from accounts where accountType ='$accountType'  AND user_name='$username'AND (user_name like '%$keyword%' or moneyIn like '%$keyword%')order by  created_at desc limit {$limitFrom}, {$limitNews}";
